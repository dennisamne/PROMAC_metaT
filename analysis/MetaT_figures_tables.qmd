...
#Identifying most abundant taxa in each plot
  ...
  NvsCUp <- c("Thaumarchaeota","Flavobacteriales","Alteromonadales","Campylobacterales")
...
bbmap_p %>%
  ...
  filter(taxa_group %in% top5) %>%
  mutate(type = case_when(type == "Up" ~ "Positive",
                          type == "Down" ~ "Negative",
                          TRUE ~ paste0(type)
                          )) %>%
ggplot(mapping = aes(x = taxa_group, y = tpm/100000, fill = treatment)) +
  geom_col(position = "dodge") +
  facet_wrap(~ type) +
...
ggplot(mapping = aes(x = order, y = tpm/100000, fill = treatment)) +
  ...
  ylab(bquote('Thaumarchaeota transcripts per million (tpm x'~10^5~')'))  
  #ylim(0, 150000)
```
