...
ggsave("../results/mesocosmVariables.png", height = 12, width = 18, units = "cm", bg = "white")
ggsave("../results/mesocosmVariables.pdf", height = 12, width = 18, units = "cm")
```
...
ggsave("../results/EGGsAndTax_eukulele.png", width = 18, units = "cm")
...
## Calculating total genes involved, and number of DE-genes in edgeR analysis

```{r}
#| label: edgeR-largescale stats
#| cache: true
#| warning: false

contrasts %>%
  distinct(seed_eggNOG_ortholog) %>%
  nrow() # Number of gene functions involved in edgeR analysis
...
```{r}
# Selecting the most abundant taxonomic levels for all contrast-responsive genes.

#Identifying most abundant taxa in each plot
  NvsCDown <- c("Rickettsiales","Burkholderiales", "SAR92", "SAR11 clade")
  NvsCUp <- c("Thaumarchaeota","Flavobacteriales","Alteromonadales","Campylobacterales","Nitrosopumilus")
  TvsCTUp <- c("Burkholderiales","Oceanospirillales", "Rickettsiales", "Nitrosomonadales")
  TNvsTTax <- c("Alteromonadales", "Oceanospirillales", "Rhodobacterales", "Hydrogenophilales")
  
  topOrd <- sort(unique(c(NvsCDown,NvsCUp,TvsCTUp,TNvsTTax)))
  
  
  #Creating the colour legend for the supplementary, same as the ALLTAX, but with addition of hydrogenophilales
  Tax_colour <- c("Alteromonadales" = "#A6CEE3",
                  "Burkholderiales" = "#1F78B4",
                  "Campylobacterales" = "yellow3",
                  "Flavobacteriales" = "#33A02C",
                  "Hydrogenophilales" = "magenta",
                  "Nitrosomonadales" = "#39FF14",
                  "Nitrosopumilus" = "brown",
                  "Oceanospirillales" = "#FDBF6F",
                  "Rhodobacterales" = "#CAB2D6",
                  "Rickettsiales" = "#6A3D9A",
...
#Getting the most abundant modules for the C-treatment contrast DE-genes
top15Module <- bbmap_p %>%
...
#NvsC contrast - C treatment genes
bbmap_p %>%
  inner_join(sample_ID %>% filter(day == '10'), by = 'sample') %>%
  filter(treatment %in% c("C")) %>% #selecting relevant treatment for the plot
  group_by(orf, treatment, timepoint) %>% # Summarising mean tpm per orf per treatment 
  summarise(tpm = mean(tpm)) %>%
  ungroup() %>%       
  inner_join(eggnogs, by = "orf") %>%
  semi_join(contrasts %>% filter(contrast == "NvsC" & type == "Down"), by = "seed_eggNOG_ortholog") %>% # Relevant enogs
  inner_join(taxonomy, by = "orf") %>% #Adding in taxonomic information
  separate_rows(KEGG_Module, sep = ",") %>%
  mutate(order_coa = coalesce(order, class, phylum)) %>%
  mutate(order_coa  = ifelse(order_coa %in% topOrd, paste0(order_coa), paste0("Other"))) %>%
  group_by(KEGG_Module, order_coa, timepoint) %>%
  summarise(tpm = sum(tpm)) %>% #Summarising tpm for the module and taxonomy
  ungroup() %>%
  mutate(Day = case_when(timepoint == "t3" ~ "day 10",
                               TRUE ~ "day 17")) %>%
  filter(KEGG_Module %in% top15Module) %>%
  left_join(module_legend, by = "KEGG_Module") %>%
  mutate(Entry = forcats::as_factor(Entry) %>% fct_reorder(tpm, .fun = "sum" ,.desc = FALSE)) %>%
  #mutate(order = fct_reorder(order_coa, tpm, .fun='sum', .desc = TRUE)) %>%
  mutate(order_coa = factor(order_coa, levels = c(topOrd, "Other"))) %>%
  #filter(order %in% levels(.$order)[1:12]) %>%
  ggplot(mapping = aes(x=Entry, y = tpm, fill = fct_relevel(order_coa, "Other", after = Inf)))  +
  geom_col() +
  theme(axis.text.x = element_text(size = 7), 
        panel.background = element_blank(),
        axis.text.y = element_text(size = 7)) +
  scale_fill_manual('Taxonomy', values = Tax_colour, drop = FALSE) +
...
#NvsC contrast - N treatment genes
#Getting the most abundant modules for the C-treatment contrast DE-genes
...
top15Module <- bbmap_p %>%
 ...
 bbmap_p %>%
  inner_join(sample_ID %>% filter(day == '10'), by = 'sample') %>%
  filter(treatment %in% c("N")) %>% #selecting relevant treatment for the plot
  group_by(orf, treatment, timepoint) %>% # Summarising mean tpm per orf per treatment 
  summarise(tpm = mean(tpm)) %>%
  ungroup() %>%       
  inner_join(eggnogs, by = "orf") %>%
  ...
  mutate(order_coa = coalesce(order, class, phylum)) %>%
  mutate(order_coa  = ifelse(order_coa %in% topOrd, paste0(order_coa), paste0("Other"))) %>%
  mutate(order_coa = ifelse(genus == "Nitrosopumilus", paste0(genus), paste0(order_coa))) %>%
  group_by(KEGG_Module, order_coa, timepoint) %>%
  summarise(tpm = sum(tpm)) %>% #Summarising tpm for the module and taxonomy
  ungroup() %>%
  mutate(Day = case_when(timepoint == "t3" ~ "day 10",
...
bbmap_p %>%
  inner_join(sample_ID %>% filter(day == '10'), by = 'sample') %>%
  filter(treatment %in% c("T")) %>% #selecting relevant treatment for the plot
  group_by(orf, treatment, timepoint) %>% # Summarising mean tpm per orf per treatment 
  summarise(tpm = mean(tpm)) %>%
  ungroup() %>%       
  inner_join(eggnogs, by = "orf") %>%
...
top15Module <- bbmap_p %>%
 ...
 bbmap_p %>%
  inner_join(sample_ID %>% filter(day == '10'), by = 'sample') %>%
  filter(treatment %in% c("TN")) %>% #selecting relevant treatment for the plot
  group_by(orf, treatment, timepoint) %>% # Summarising mean tpm per orf per treatment 
  summarise(tpm = mean(tpm)) %>%
  ungroup() %>%       
  inner_join(eggnogs, by = "orf") %>%
...
### Now add all the other phyla and unused levels to one plot, and use only that legend, to merge them well!

(NvsC_CDown + theme(legend.position = "none")) + 
(NvsC_NUp + theme(legend.position = "none")) + 
(TvsC_TUp + theme(legend.position = "none"))  + 
 TNvsT_TNUp  +
  #guides(fill = guide_legend(title.position = "top")) +
  plot_annotation(tag_levels = list(c("C","N","T","TN"))) +
  plot_layout(guides = "collect") & 
...
ggsave("../results/contModu.png", width = 20, height = 21 ,unit = "cm")
ggsave("../results/contModu.pdf", width = 20, height = 21 ,unit = "cm")
```

## Fig interaction taxonomy
...
ggplot(mapping = aes(x = order, y = tpm/100000, fill = treatment)) +
  ...
  ylab(bquote('Nitrosopumilus transcripts per million (tpm x'~10^5~')'))  
  #ylim(0, 150000)
```
...
ggsave("../results/InteractAndArch.png")
```

# Looking at interaction effect eggNOGs
...
ggsave("../results/nested_taxonomies.png", plot = p_combined, height = 20, width = 24, units = "cm")
...
# So the overlaps are as such:
  ...
  #Hao is in both Nitrification and complete nitrification.
...
## Looking at the Energy metabolism in nitrosopumilus in the N-treatment

We see a gene involved in glycolysis/gluconeogenesis Another one in the Hydroxypropionate-hydroxybutylate cycle which is a carbon fixation pathway. Another one is a nitrate reductase (What we see is denitrificaiton in the module level) An ammonia oxidase!
...
ArchEnMet <- bbmap_p %>%
  ...
  #mutate(order_coa  = ifelse(order_coa %in% topOrd, paste0(order_coa), paste0("Other"))) %>%
  mutate(KEGG_level_2 = ifelse(KEGG_ko == "ko:K03320", paste0("Ammonium transporter"),paste0(KEGG_level_2))) %>%
  filter(order_coa == "Nitrosopumilus" & KEGG_level_2 == "Energy metabolism") %>%
  inner_join(eggnogs %>% distinct(seed_eggNOG_ortholog, KEGG_ko,Preferred_name, narr_og_desc))

write_tsv(ArchEnMet, "../results/ArchEnMet.tsv")
```

## Do not keep

```{r}
#PLotta amoA och amtB utanb kontraster för Nitrosopumilus

## K10944  - amoA
## 
```

## Looking into the energy metabolism in Nitrosopumilus by the interaction effect in the C-N treatments

This is interesting, as one gene (K00261) glutamate dehydrogenase is involved in ammonia assimilation\
Ref: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC166173/

```{r}
hmm <- bbmap_p %>%
  inner_join(sample_ID, by = "sample") %>%
  semi_join(sample_ID %>% filter(day == '10'), by = 'sample') %>%
  filter(treatment %in% c("N","C")) %>% #selecting relevant treatments for the plot
  group_by(orf, treatment,timepoint) %>% # Summarising mean tpm per orf per treatment and timepoint
  summarise(tpm = mean(tpm)) %>%
  ungroup() %>%       
  inner_join(orf_KEGG2_key %>% distinct(orf, KEGG_level_2), by = "orf") %>%
  inner_join(eggnogs %>% distinct(orf, seed_eggNOG_ortholog, KEGG_ko), by = "orf") %>%
  semi_join(contrasts %>% filter(contrast == "interaction" & type %in% c("Up","Down")), by = "seed_eggNOG_ortholog") %>% # Relevant enogs
  left_join(taxonomy, by = "orf") %>% #Adding in taxonomic information
  mutate(order_coa = coalesce(order, class, phylum)) %>%
  mutate(order_coa = ifelse(phylum == "Thaumarchaeota" & genus == "Nitrosopumilus", paste0(genus), paste0(order_coa))) %>%
  mutate(order_coa  = ifelse(order_coa %in% topOrd, paste0(order_coa), paste0("Other"))) %>%
...
p1 + p2 + p3 + p4 + plot_annotation(tag_levels = list(c('C', 'N', "T","TN"))) #+ plot_layout(guides = "collect")

ggsave("../results/top30_eggnogs.pdf")
```



```{r}
# Preparing the plotting here
genes_of_interest <- c("")

TNvsT %>%
  filter(Preferred_name %in% genes_of_interest) %>%
  ggplot(mapping = aes(x = Preferred_name, y = logCPM)) +
  geom_col() +
