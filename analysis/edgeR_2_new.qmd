...
An edgeR test was run to compare the different contrasts of interest. The setup was done in a 2x2 factorial design with the variables temperature (1 and 10°C) and DOM (addition and no addition). The combination of these two factors produced four treatments:

Cold_control (**C**), Cold_nutrient (**N**), Warm_control (**T**) , Warm_nutrient (**TN**). To investigate the effect of nutrient addition on microbial community gene expression at different temperature regimes, the contrasts compared are:\
$N - C$\
$TN - T$\
$(TN - T) - (N - C)$\
...
kable(contrasts %>%
  ...
  add_header_above(c("EdgeR contrast overview" = 4), font_size = 18)
```
...
#| fig-cap: The most abundant upregulated genes in the three contrasts
#| fig-subcap: 
#|   - "NvsC contrast upregulated genes"
#|   - "TNvsT contrast upregulated genes"
#|   - "Interaction contrast upregulated genes"
#| layout-ncol: 3
#| warning: false
...
Would it be a good idea to combine this plot with the table of the output of number of genes (e.g Up/Down/non-DE) per contrast, and put their number in a parenthesis on the x-axis?
