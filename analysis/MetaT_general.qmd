...
mesocosm_variables %>%
  ...
  mutate(variable = factor(variable,
                           ...
                           labels = c("Chlorophyll~a~(µg~dm^{-3})",
                                      ...
                                      "Prokaryotic~abundance~(cells~dm^{-3})",
                                      "Prokaryotic~growth~(cells~dm^{-3}~d^{-1})"
                                      )
                           )) %>%
...
Questions: How many of DE-transcripts are part of *Nitrosopumilus*? If many, what are the characterizing differences?
...
#Summarising the larger KEGG categories accordign to their structure in https://www.genome.jp/kegg/pathway.html I did leave out the 1.0 category global and overview maps, as it is so wide, and obscures the rest of the patterns due to high tpm values, and doesn't feel very informative by itself...
...
orf_KEGG2_key <- eggnogs %>%
  ...
  mutate(KEGG_level_2 = case_when(
    ...
    KEGG_Pathway %in% Cellular_community_prokaryotes ~ "Cellular_community_prokaryotes",
    KEGG_Pathway %in% Cell_motility ~ "Cell_motility"
  ))
  
#Using hte most abundant taxonomic levels instead
...
bbmap_p %>%
 ...
 mutate(KEGG_level_2 = ifelse(KEGG_ko == "ko:K03320", paste0("ammonium transporter"),paste0(KEGG_level_2))) %>%
  ...
  mutate(KEGG_level_2 = forcats::as_factor(KEGG_level_2) %>% fct_reorder(tpm, .fun = "sum" ,.desc = FALSE)) %>%
   mutate(Day = case_when(timepoint == "t3" ~ "day 10",
                               TRUE ~ "day 17")) %>%
   ggplot(mapping = aes(x=KEGG_level_2, y = tpm, fill = order_coa))  +
  geom_col() +
  theme(axis.text.x = element_text(size = 7), 
        panel.background = element_blank(),
        axis.text.y = element_text(size = 7)) +
  scale_fill_manual('Taxonomy', values = GEOM_COL_COLOURS) +
  ylab("transcripts per million") +
  xlab("") +
  coord_flip() +
  theme(strip.background = element_blank())
...
Sorting the Differential gene expression into functional categories. Nutrient addition produced a more even response between categories, regardless of temperature. The treatments without nutrient addition instead had a large response in the translation category. \### The C More than half of the differential expression in the C-treatment was characterised by Thaumarchaeal ammonium transporter expression (Fig ContKegg). This was followed by a large set of genes involved in translation which was constituted mainly by Burkholderiales. \### The N The N treatment had an even distribution among the most abunfant taxa and categories, the two largest categories were carbohydrate and energy metabolism. Carbohydrate metabolism was expressed by Altermonadales, Burkholderiales, the same taxa were also the main expressors of the energy metabolism, with the addition of Thaumarchaeota. \### The T The T expression had a large response in translation as compared to the other metabolic categories. This expressin was constituted mainly by Burkholderiales, Oceanospirillales and Rhodobacterales.

### The TN

Expression in the TN was evenly distributed among Energy metabolism, translation and carbohydrate metabolism, this expression was dominated by Oceanospirillales.

```{r}
 NvsC_NUp  + NvsC_CDown  + TvsC_TUp + TNvsT_TNUp +
  plot_annotation(tag_levels = list(c("a","b","c","d"))) +
  plot_layout(guides = "collect") & 
  theme(legend.position = "bottom",
        axis.title.x = element_text(size = 7, colour = "black", face = "bold"),
...
ggsave("../results/pathway_level2.png")
```
...
## Thaumarhaeal DE gene abundances

```{r}
ThaumGeneInter <- bbmap_p %>%
  inner_join(sample_ID %>% filter(day == "10"), by = "sample") %>%
  group_by(orf, treatment,timepoint) %>% # Summarising mean tpm per orf per treatment and timepoint
  summarise(tpm = mean(tpm)) %>%
  ungroup() %>%
  ...
  slice(1:50) %>%
  distinct(orf) %>%
  left_join(eggnogs %>% distinct(orf, KEGG_ko), by = "orf")


##Most avbundant thamgenes in C-treatment

most_abThaum <-  bbmap_p %>%
  inner_join(sample_ID %>% filter(day == "10"), by = "sample") %>%
  filter(treatment == "C") %>%
  group_by(orf, treatment,timepoint) %>% # Summarising mean tpm per orf per treatment and timepoint
  summarise(tpm = mean(tpm)) %>%
  ungroup() %>%
  inner_join(eggnogs %>% distinct(orf, KEGG_ko, best_og_desc, seed_eggNOG_ortholog, KEGG_Pathway, KEGG_Module), by = "orf") %>%
  inner_join(taxonomy %>% filter(phylum %in% c("Thaumarchaeota")), by = "orf") 
   

```

# Interaction genes boxplot - not showing the patterns clearly, instead try paired comparisons, here the patterns are visible, but not completely clear. Is it due to tpm vs logCPM comparisons, lowly abundant genes, or the mix of values?
...
bbmap_p %>%
  ...
  inner_join(eggnogs %>% distinct(orf, seed_eggNOG_ortholog), by = "orf") %>%
  inner_join(contrasts %>% filter(contrast == "interaction" & type %in% c("Up","Down")), by = "seed_eggNOG_ortholog") %>%
  group_by(seed_eggNOG_ortholog, treatment, type) %>%
  summarise(tpm = sum(tpm)) %>%
  ungroup() %>%
  ggplot(mapping = aes(x = type, y = log(tpm), fill = treatment)) +
  geom_violin() 
...
## The Burkholderiales C-abundant genes

```{r}
## Fix this to Burkholderiales
 bbmap_p %>%
  inner_join(sample_ID %>% filter(day == "10"), by = "sample") %>%
  group_by(orf, treatment,timepoint) %>% # Summarising mean tpm per orf per treatment and timepoint
  summarise(tpm = mean(tpm)) %>%
  ungroup() %>%
  inner_join(eggnogs %>% distinct(orf, seed_eggNOG_ortholog, KEGG_Pathway), by = "orf") %>%
  inner_join(contrasts %>% filter(contrast == "NvsC" & type %in% c("Down")), by = "seed_eggNOG_ortholog") %>%
  inner_join(taxonomy %>% filter(phylum %in% c("Thaumarchaeota")), by = "orf") %>%
  separate_rows(KEGG_Pathway, sep = ",") %>%
  mutate(KEGG_Pathway = gsub("[a-z]*","", KEGG_Pathway)) %>%
  distinct(KEGG_Pathway, treatment, timepoint, orf, tpm, type) %>% #Getting rid of duplicates when separating
...
https://www.kegg.jp/entry/K22169 - an oxidoreductase https://www.kegg.jp/entry/K18779 - tRNA-ribosyltransferase https://www.kegg.jp/entry/K18603 - acetyl-CoA/propionyl-CoA carboxylase involved in carbon fixation https://www.kegg.jp/entry/K18602 - malonic semialdehyde reductase ,involved in carbon fixation as well https://www.kegg.jp/entry/K16011 - invovled in many things https://www.kegg.jp/entry/K15019 - also involved in carbon fixation pathways https://www.kegg.jp/entry/K15016 - involved in carbon fixation https://www.kegg.jp/entry/K14728 - oxidoreductase
...
### The most abundant genes in the C-fraction (NvsC) including taxonomy and function
...
DEGenes_C <- bbmap_p %>%
  ...
  ungroup() %>%
  inner_join(eggnogs %>% distinct(orf, seed_eggNOG_ortholog), by = "orf") %>%
  semi_join(contrasts %>% filter(contrast == "NvsC" & type %in% c("Down")), by = "seed_eggNOG_ortholog") %>% #Selecting only relevant orfs
  filter(treatment == "C") %>%
  inner_join(taxonomy, by = "orf") %>%
  inner_join(eggnogs, by = "orf")
```

### The most abundant genes in the N-fraction (NvsC) including taxonomy and function
...
DEGenes_N <- bbmap_p %>%
  ...
  ungroup() %>%
  inner_join(eggnogs %>% distinct(orf, seed_eggNOG_ortholog), by = "orf") %>%
  semi_join(contrasts %>% filter(contrast == "NvsC" & type %in% c("Up")), by = "seed_eggNOG_ortholog") %>% #Selecting only relevant orfs
  filter(treatment == "N") %>%
  inner_join(taxonomy, by = "orf") %>%
  inner_join(eggnogs, by = "orf")

DEGenes_N %>%
  mutate(orf = factor(orf) %>% forcats::fct_reorder(tpm, .desc = TRUE)) %>%
  ggplot(mapping = aes(x = orf, y = tpm)) +
  geom_point() +
  theme(axis.text.x = element_blank())
```

### The most abundant genes in the T-fraction (TvsC) including taxonomy and function
...
DEGenes_T <- bbmap_p %>%
  ...
  ungroup() %>%
  inner_join(eggnogs %>% distinct(orf, seed_eggNOG_ortholog), by = "orf") %>%
  semi_join(contrasts %>% filter(contrast == "TvsC" & type %in% c("Up")), by = "seed_eggNOG_ortholog") %>% #Selecting only relevant orfs
  filter(treatment == "T") %>%
  inner_join(taxonomy, by = "orf") %>%
  inner_join(eggnogs, by = "orf")
...
### The most abundant genes in the TN-fraction (TNvsT) including taxonomy and function

\`\`\`{r} DEGenes_TN \<- bbmap_p %\>% inner_join(sample_ID %\>% filter(day == "10"), by = "sample") %\>% group_by(orf, treatment,timepoint) %\>% \# Summarising mean tpm per orf per treatment and timepoint summarise(tpm = mean(tpm)) %\>% ungroup() %\>% inner_join(eggnogs %\>% distinct(orf, seed_eggNOG_ortholog), by = "orf") %\>% semi_join(contrasts %\>% filter(contrast == "TNvsT" & type %in% c("Up")), by = "seed_eggNOG_ortholog") %\>% #Selecting only relevant orfs filter(treatment == "TN") %\>% inner_join(taxonomy, by = "orf") %\>% inner_join(eggnogs, by = "orf")

DEGenes_N %\>% mutate(orf = factor(orf) %\>% forcats::fct_reorder(tpm, .desc = TRUE)) %\>% ggplot(mapping = aes(x = orf, y = tpm)) + geom_point() + theme(axis.text.x = element_blank())

# How many genes in nitrogen metabolism??
```{r}
contrasts %>%
  filter(contrast == "interaction" & type == "Down") %>%
  inner_join(eggnogs %>% distinct(orf, seed_eggNOG_ortholog), by = "seed_eggNOG_ortholog") %>%
  inner_join(eggnogs %>% distinct(orf, KEGG_Module), by = "orf") %>%
  separate_rows(KEGG_Module, sep = ",") %>%
  distinct(KEGG_Module, seed_eggNOG_ortholog) %>% 
  inner_join(module_legend, by = "KEGG_Module") %>%
  filter(KEGG_Module %in% c("M00804","M00528")) %>%
  distinct(seed_eggNOG_ortholog) # Seven different seed orthologous groups.
